...
public class Kappa extends Entity {
	...
	public Kappa(float x, float y, float velX, float velY) {
		super(x, y, velX, velY);
		try {
			this.texture = new Image("res/kappa.png_large");
			this.texture = this.texture.getScaledCopy(100, 100);
	...
	public void collision(Kappa other) {
		...
		if (this.x > other.x && this.x < other.x + other.texture.getWidth()
				&& (this.y + this.texture.getHeight() > other.y
						&& this.y + this.texture.getHeight() < other.y + other.texture.getHeight()
						|| this.y > other.y && this.y < other.y + other.texture.getHeight())) {
			this.react(0);
			other.react(0);
		}
		// Collision verticale
		if (this.y > other.y && this.y < other.y + other.texture.getHeight()
				&& (this.x + this.texture.getWidth() > other.x
						&& this.x + this.texture.getWidth() < other.x + other.texture.getWidth()
						|| this.x > other.x && this.x < other.x + other.texture.getWidth())) {
			this.react(1);
			other.react(1);
		}
	}
